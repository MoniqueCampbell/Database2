...
def cpost(userid):
    if request.method == 'GET':
        form = CPostForm()
        return render_template('cpost.html', form = form, userid=userid)
...
def vpost(userid):
    if request.method == 'GET':
        ...
        cur.execute("SELECT post_id,description,post_date,post_time FROM Post WHERE post_id in (SELECT post_id FROM Submits WHERE user_id='{}')".format(userid))
        posts=cur.fetchall()
